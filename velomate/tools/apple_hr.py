...
def parse_apple_json(raw: bytes, ignore_implausible: bool = True, min_hr: int = 30, max_hr: int = 240) -> list[HrPoint]:
    ...
    if isinstance(data, dict):
        entries = data.get("heartRateData") or data.get("heart_rate") or data.get("samples") or data.get("data")
        if entries is None:
            # Support top-level single-object payloads like {"timestamp": "...", "hr": 142}
            entries = [data]
    else:
        entries = data
